  ...
  private async getWeeklyAppointmentChart(
    ...
    const employeeFilter = employeeId
      ? Prisma.sql`AND id IN (
          SELECT "appointmentId" FROM "Participant" p 
          JOIN "AppointmentService" s ON p.id = s."participantId" 
          WHERE s."employeeId" = ${employeeId}
        )`
      : Prisma.empty;

    const rawData = await this.prisma.$queryRaw<any[]>`
      SELECT TO_CHAR(date, 'Dy') as day_name, COUNT(id)::INT as count
      FROM "Appointment"
    ...
    return rawData.map((item) => ({ day: item.day_name, count: item.count }));
  }
